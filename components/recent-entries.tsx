    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        toast({
          title: "Error",
          description:
            "Failed to load recent entries. Please refresh the page.",
          variant: "destructive",
        });
      }
    } catch (error) {
        ...
        {entries.length > 0 ? (
          entries.map((entry) => {
            ...
            return (
              <div
                key={entry.id}
                className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors gap-3 relative group"
              >
                <div className="flex items-start gap-3">
                  <div
                    className={`mt-1.5 h-2.5 w-2.5 rounded-full ring-2 ring-background ${entry.status === "active" ? "bg-green-500 animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.5)]" : "bg-muted-foreground/30"}`}
                  />
                  <div className="space-y-1">
                    <div className="font-semibold text-sm flex items-center gap-2">
                      ...
                      {entry.status === "active" && (
                        <Badge
                          variant="default"
                          className="text-[10px] h-5 px-1.5 bg-green-500 hover:bg-green-600 border-none shadow-sm"
                        >
                          Active
                        </Badge>
                      )}
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-1.5">
                      <span className="inline-flex items-center justify-center min-w-[60px] px-1.5 py-0.5 rounded-md bg-muted/50 border border-border/50 text-center font-mono text-[11px] font-medium text-foreground/80">
                        {formatTimeBR(startTime)}
                      </span>
                      <span className="text-muted-foreground/40 text-[10px] uppercase font-bold tracking-wider">
                        to
                      </span>
                      <span className="inline-flex items-center justify-center min-w-[60px] px-1.5 py-0.5 rounded-md bg-muted/50 border border-border/50 text-center font-mono text-[11px] font-medium text-foreground/80">
                        {endTime ? formatTimeBR(endTime) : "..."}
                      </span>
                    </div>
                  </div>
                </div>
